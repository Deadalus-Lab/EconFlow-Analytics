...
# Measured 2026-08-23: 7 files, 52 references. The floors sit below that so an
# ordinary addition does not trip them, and far enough above zero that a glob
# that stopped matching cannot pass. The file floor stays exactly one below the
...
# It rose from 4 to 6 when claude.yml and claude-review.yml were restored.
MIN_FILES = 6
MIN_REFERENCES = 20
