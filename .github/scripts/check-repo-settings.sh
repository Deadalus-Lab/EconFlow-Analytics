...
# cannot run under the workflow's own GITHUB_TOKEN. Every expectation is a
# decision recorded in docs/decisions/marketplace-apps.md, ARCHITECTURE.md
# 11.1/13 or .github/rulesets/*.json; change the decision first, then the line.
#
...
used=$(grep -rhoE 'uses: [^ ]+' .github/workflows .github/actions 2>/dev/null | sed 's/uses: //' | grep -v '^\./' | grep -v '^actions/' | cut -d@ -f1 | sort -u)
for a in $used; do
...
topics=$(rf topics)
case "$topics" in
  *"'r'"*) printf '  FAIL  %-52s %s\n' "topics" "still advertise 'r'"; fail=1 ;;
  *python*) printf '  ok    %-52s %s\n' "topics" "$topics" ;;
  *) printf '  FAIL  %-52s %s (python missing)\n' "topics" "$topics"; fail=1 ;;
esac

if [ "$fail" -eq 0 ]; then
  echo "repository settings match the decisions."
else
  echo "repository settings DRIFTED from the decisions." >&2; exit 1
fi
