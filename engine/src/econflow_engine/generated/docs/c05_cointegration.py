...
NODE_DOCS: dict[str, dict[str, Any]] = {
    'ardl_auto': {
        'fn': 'ardl_auto',
        'description': 'ardl_auto -- category 05-cointegration, method card #25.',
    ...
    'ardl_bounds_f': {
        'fn': 'ardl_bounds_f',
        'description': 'ardl_bounds_f -- category 05-cointegration, method card #25.',
    ...
    'ardl_bounds_t': {
        'fn': 'ardl_bounds_t',
        'description': 'ardl_bounds_t -- category 05-cointegration, method card #25.',
    ...
    'ardl_coint_eq': {
        'fn': 'ardl_coint_eq',
        'description': 'ardl_coint_eq -- category 05-cointegration, method card #25.',
    ...
    'ardl_fit': {
        'fn': 'ardl_fit',
        'description': 'ardl_fit -- category 05-cointegration, method card #25.',
    ...
    'ardl_multipliers': {
        'fn': 'ardl_multipliers',
        'description': 'ardl_multipliers -- category 05-cointegration, method card #25.',
    ...
    'ardl_recm': {
        'fn': 'ardl_recm',
        'description': 'ardl_recm -- category 05-cointegration, method card #25.',
        'args': {'object': "Handle to an 'ardl'/'uecm' model (from ardl_fit/ardl_auto).", 'case': 'PSS case of deterministic terms; integer 1..5.'},
        'input_example': {'object': '<raw_handle>', 'case': 1},
    },
    'cmo_cointegration': {
        'fn': 'cmo_cointegration',
        'description': 'cmo_cointegration -- category 05-cointegration, method card #141.',
    ...
    'cmo_stationarity': {
        'fn': 'cmo_stationarity',
        'description': 'cmo_stationarity -- category 05-cointegration, method card #141.',
    ...
    'creg_dols': {
        'fn': 'creg_dols',
        'description': 'creg_dols -- category 05-cointegration, method card #27.',
    ...
    'creg_fmols': {
        'fn': 'creg_fmols',
        'description': 'creg_fmols -- category 05-cointegration, method card #27.',
    ...
    'creg_imols': {
        'fn': 'creg_imols',
        'description': 'creg_imols -- category 05-cointegration, method card #27.',
    ...
    'dyn_ardl': {
        'fn': 'dyn_ardl',
        'description': 'dyn_ardl -- category 05-cointegration, method card #26.',
        'args': {'formula': "ARDL/ECM formula (e.g. 'y ~ x1 + x2').", 'data': 'Handle to a DataFrame (NOT ts; dynardl builds lags/diffs on its own).', 'diffs': 'Variable names in first differences (array of strings).', 'levels': 'Variable names in levels (array of strings).', 'ec': 'True = error-correction parameterization (required for dyn_pss_bounds).', 'trend': 'True = deterministic trend (default False).', 'constant': 'True = constant term (default True).', 'simulate': 'True = stochastic simulation of the counterfactual response (chart-data).', 'shockvar': 'Variable to shock (required when simulate=True; one of the RHS).', 'sims': 'Monte Carlo replications of the simulation (default 1000).', 'range': 'Horizon of the response path (default 20).'},
        'input_example': {'formula': 'y ~ x', 'data': '<df_handle>', 'ec': False, 'trend': False, 'constant': True, 'simulate': False, 'sims': 1000, 'range': 20},
    },
    'dyn_autocorr': {
        'fn': 'dyn_autocorr',
        'description': 'dyn_autocorr -- category 05-cointegration, method card #26.',
        'args': {'object': "Handle to a 'dynardl' model (from dyn_ardl).", 'bg_type': 'Breusch-Godfrey statistic (default Chisq).', 'order': 'AR order of the autocorrelation test (default 1).'},
        'input_example': {'object': '<raw_handle>', 'order': 1},
    },
    'dyn_pss_bounds': {
        'fn': 'dyn_pss_bounds',
        'description': 'dyn_pss_bounds -- category 05-cointegration, method card #26.',
        'args': {'object': "Handle to a 'dynardl' estimated with ec=True (from dyn_ardl)."},
        'input_example': {'object': '<raw_handle>'},
    },
    'fcv_boot_rank': {
        'fn': 'fcv_boot_rank',
        'description': 'fcv_boot_rank -- category 05-cointegration, method card #139.',
        'args': {'x': 'Handle to a multivariate system (matrix/panel, >=2 columns).', 'k': 'Number of lags; integer >=0 (default 1).', 'r1': 'Rank under H0; integer 0..p (default 0).', 'r2': 'Rank under H1; integer 0..p, r2>r1 (default 1).', 'B': 'Number of bootstrap samples; integer >=1 (default 99).', 'seed': 'Seed (REQUIRED — wild bootstrap, stochastic).', 'grid_search': 'Grid search for d,b (default False).'},
        'input_example': {'x': '<matrix_handle>', 'k': 1, 'r1': 0, 'r2': 1, 'B': 99, 'seed': 1, 'grid_search': False},
    },
    'fcv_estimate': {
        'fn': 'fcv_estimate',
        'description': 'fcv_estimate -- category 05-cointegration, method card #139.',
        'args': {'x': 'Handle to a multivariate system (matrix/panel, >=2 columns, fractionally integrated).', 'k': 'Number of lags in the system; integer >=0 (default 1).', 'r': 'Cointegration rank; integer 0..p (default 1).', 'db_min': 'Lower bound for (d,b); scalar or length-2 (default 0.01).', 'db_max': 'Upper bound for (d,b); scalar or length-2 (default 2.0).', 'restrict_db': 'Enforcement of the d=b constraint (default True).', 'constrained': 'Enforcement of dbMax>=d>=b>=dbMin (default False).', 'level_param': 'Inclusion of a level parameter (default True).', 'r_constant': 'Restricted constant in the cointegrating relation (default False).', 'unr_constant': 'Unrestricted constant (default False).', 'grid_search': 'Grid search for d,b (expensive accuracy-refinement; default False).'},
    ...
    'fcv_lag_select': {
        'fn': 'fcv_lag_select',
        'description': 'fcv_lag_select -- category 05-cointegration, method card #139.',
        'args': {'x': 'Handle to a multivariate system (matrix/panel, >=2 columns).', 'kmax': 'Maximum number of lags; integer >=1 (default 2).', 'r': 'Cointegration rank; integer 0..p (default p — over-specify).', 'order': 'Serial correlation order for the white-noise tests; >=1 (default 2).', 'grid_search': 'Grid search for d,b (default False).'},
        'input_example': {'x': '<matrix_handle>', 'kmax': 2, 'order': 2, 'grid_search': False},
    },
    'fcv_rank_test': {
        'fn': 'fcv_rank_test',
        'description': 'fcv_rank_test -- category 05-cointegration, method card #139.',
        'args': {'x': 'Handle to a multivariate system (matrix/panel, >=2 columns).', 'k': 'Number of lags; integer >=0 (default 1).', 'restrict_db': 'Enforcement of d=b (default True).', 'constrained': 'Enforcement of dbMax>=d>=b>=dbMin (default False).', 'grid_search': 'Grid search for d,b (default False).'},
        'input_example': {'x': '<matrix_handle>', 'k': 1, 'restrict_db': True, 'constrained': False, 'grid_search': False},
    },
    'nar_bounds': {
        'fn': 'nar_bounds',
        'description': 'nar_bounds -- category 05-cointegration, method card #140.',
        'args': {'object': "Handle to a 'nardl' model (from nar_fit)."},
        'input_example': {'object': '<raw_handle>'},
    },
    'nar_fit': {
        'fn': 'nar_fit',
        'description': 'nar_fit -- category 05-cointegration, method card #140.',
    ...
    'vecm_rank_select': {
        'fn': 'vecm_rank_select',
        'description': 'vecm_rank_select -- category 05-cointegration, method card #24.',
        'args': {'data': 'Handle to a multivariate system (matrix/panel, >=2 columns).', 'lag_max': 'Maximum lag for the search; positive integer.', 'include': 'Deterministic terms (default const).'},
        'input_example': {'data': '<matrix_handle>', 'lag_max': 1},
    },
    'vecm_rank_test': {
        'fn': 'vecm_rank_test',
        'description': 'vecm_rank_test -- category 05-cointegration, method card #24.',
    ...
    'coint_johansen': {
        'fn': 'coint_johansen',
        'description': 'coint_johansen -- category 05-cointegration, method card #23.',
        'args': {'x': 'Handle to a multivariate system (matrix/panel, >=2 columns I(1)).', 'type': 'Johansen statistic (default eigen = max-eigenvalue).', 'ecdet': 'Deterministic term in the cointegrating relation (default none).', 'K': 'Lag order in levels (VAR); integer >=2 (default 2).', 'spec': 'VECM parameterization (default longrun).'},
        'input_example': {'x': '<matrix_handle>', 'K': 2},
    },
    'coint_phillips_ouliaris': {
        'fn': 'coint_phillips_ouliaris',
        'description': 'coint_phillips_ouliaris -- category 05-cointegration, method card #23.',
        'args': {'z': 'Handle to a multivariate system (matrix/panel, >=2 columns).', 'demean': 'Deterministic terms in the regression (default none).', 'lag': 'Long-run variance lag selection (default short).', 'type': 'Phillips-Ouliaris statistic (default Pu).'},
        'input_example': {'z': '<matrix_handle>'},
    },
    'coint_johansen_vecm': {
        'fn': 'coint_johansen_vecm',
        'description': 'coint_johansen_vecm -- category 05-cointegration, method card #23.',
        'args': {'z': "Handle to a 'Johansen' object (from coint_johansen).", 'r': 'Cointegration rank; integer 1..(P-1) (default 1).'},
        'input_example': {'z': '<raw_handle>', 'r': 1},
    },
    'ci_gregory_hansen': {
        'fn': 'ci_gregory_hansen',
        'description': 'ci_gregory_hansen -- category 05-cointegration, method card #432.',
        'args': {'y': 'Dependent series.', 'x': 'Regressor series.', 'model': 'Break specification.', 'trim': 'Fraction trimmed from each end of the break search.', 'lags': 'Lag order for the ADF regressions.', 'alpha': 'Significance level.'},
        'input_example': {'y': '<series_handle>', 'x': '<multiseries_handle>', 'model': 'level', 'trim': 0.15, 'alpha': 0.05},
    },
    'ci_vecm_restrict': {
        'fn': 'ci_vecm_restrict',
        'description': 'ci_vecm_restrict -- category 05-cointegration, method card #433.',
        'args': {'fit': 'Handle to a fitted VECM.', 'beta_restriction': 'Restriction matrix on beta.', 'alpha_restriction': 'Restriction matrix on alpha.', 'alpha': 'Significance level.'},
        'input_example': {'fit': '<raw_handle>', 'alpha': 0.05},
    },
    'ci_weak_exogeneity': {
        'fn': 'ci_weak_exogeneity',
        'description': 'ci_weak_exogeneity -- category 05-cointegration, method card #433.',
        'args': {'fit': 'Handle to a fitted VECM.', 'alpha': 'Significance level.'},
        'input_example': {'fit': '<raw_handle>', 'alpha': 0.05},
    },
    'ci_panel_cointegration': {
        'fn': 'ci_panel_cointegration',
        'description': 'ci_panel_cointegration -- category 05-cointegration, method card #434.',
        'args': {'data': 'Panel data.', 'y': 'Dependent column.', 'x': 'Regressor columns.', 'unit': 'Unit identifier.', 'time': 'Time identifier.', 'test': 'Test family.', 'trend': 'Deterministic terms.', 'nboot': 'Number of bootstrap replications.', 'seed': 'Seed for the random number generator.', 'alpha': 'Significance level.'},
        'input_example': {'data': '<df_handle>', 'y': '...', 'unit': '...', 'time': '...', 'test': 'pedroni', 'trend': 'c', 'nboot': 0, 'alpha': 0.05},
    },
    'ci_threshold_ecm': {
        'fn': 'ci_threshold_ecm',
        'description': 'ci_threshold_ecm -- category 05-cointegration, method card #435.',
        'args': {'y': 'Dependent series.', 'x': 'Regressor series.', 'model': 'Threshold specification.', 'threshold': 'Threshold; omitted = estimated by grid search.', 'trim': 'Fraction trimmed from each end of the threshold grid.', 'nboot': 'Number of bootstrap replications.', 'seed': 'Seed for the random number generator; required for reproducibility.', 'alpha': 'Significance level.'},
        'input_example': {'y': '<series_handle>', 'x': '<multiseries_handle>', 'model': 'tar', 'trim': 0.15, 'nboot': 999, 'seed': 1, 'alpha': 0.05},
    },
    'ci_canonical_regression': {
        'fn': 'ci_canonical_regression',
        'description': 'ci_canonical_regression -- category 05-cointegration, method card #436.',
        'args': {'y': 'Dependent series.', 'x': 'Regressor series.', 'method': 'Estimator.', 'kernel': 'Kernel for the long-run covariance.', 'bandwidth': 'Bandwidth; omitted = automatic.', 'leads_lags': 'Leads and lags for the dynamic estimator.', 'conf_level': 'Confidence level for intervals.'},
        'input_example': {'y': '<series_handle>', 'x': '<multiseries_handle>', 'method': 'ccr', 'kernel': 'bartlett', 'leads_lags': 2, 'conf_level': 0.95},
    },
}
