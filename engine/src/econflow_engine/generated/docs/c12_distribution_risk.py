...
NODE_DOCS: dict[str, dict[str, Any]] = {
    'cop_fit': {
        'fn': 'cop_fit',
        'description': 'cop_fit -- category 12-distribution-risk, method card #197.',
    ...
    'cop_gof': {
        'fn': 'cop_gof',
        'description': 'cop_gof -- category 12-distribution-risk, method card #197.',
    ...
    'cop_tail_index': {
        'fn': 'cop_tail_index',
        'description': 'cop_tail_index -- category 12-distribution-risk, method card #197.',
    ...
    'er_expectiles': {
        'fn': 'er_expectiles',
        'description': 'er_expectiles -- category 12-distribution-risk, method card #192.',
    ...
    'er_mes': {
        'fn': 'er_mes',
        'description': 'er_mes -- category 12-distribution-risk, method card #192.',
    ...
    'er_tail_index': {
        'fn': 'er_tail_index',
        'description': 'er_tail_index -- category 12-distribution-risk, method card #192.',
    ...
    'es_backtest': {
        'fn': 'es_backtest',
        'description': 'es_backtest -- category 12-distribution-risk, method card #194.',
    ...
    'es_joint_reg': {
        'fn': 'es_joint_reg',
        'description': 'es_joint_reg -- category 12-distribution-risk, method card #194.',
    ...
    'esg_martingale_test': {
        ...
        'description': 'esg_martingale_test -- category 12-distribution-risk, method card #199.',
        'args': {'object': 'Handle to esg_simulate_diffusion.object (list model/params/T/time/paths). Gate: model ∈ {vasicek,cir} (only these have a closed-form bond price).', 'conf_level': 'Confidence level of the MC interval for the within_ci check, in (0,1) (default 0.95).'},
        'input_example': {'object': '<raw_handle>', 'conf_level': 0.95},
    },
    'esg_simulate_diffusion': {
        'fn': 'esg_simulate_diffusion',
        'description': 'esg_simulate_diffusion -- category 12-distribution-risk, method card #199.',
    ...
    'evt_fit': {
        'fn': 'evt_fit',
        'description': 'evt_fit -- category 12-distribution-risk, method card #191.',
    ...
    'evt_return_level': {
        'fn': 'evt_return_level',
        'description': 'evt_return_level -- category 12-distribution-risk, method card #191.',
        'args': {'object': 'Handle to an fevd fit (from evt_fit.object).', 'return_periods': 'Vector of return periods in period_basis units· each value > 1 (default [10,50,100]).', 'do_ci': 'True (default) -> normal-approx (delta) CIs· False -> only point estimates.', 'conf_level': 'Confidence level in (0,1) for the CIs (default 0.95).'},
        'input_example': {'object': '<raw_handle>', 'do_ci': True, 'conf_level': 0.95},
    },
    'evt_threshold_diag': {
        'fn': 'evt_threshold_diag',
        'description': 'evt_threshold_diag -- category 12-distribution-risk, method card #191.',
    ...
    'exr_expectile': {
        'fn': 'exr_expectile',
        'description': 'exr_expectile -- category 12-distribution-risk, method card #193.',
    ...
    'exr_expectile_reg': {
        ...
        'description': 'exr_expectile_reg -- category 12-distribution-risk, method card #193.',
    ...
    'fd_bootstrap_ci': {
        'fn': 'fd_bootstrap_ci',
        'description': 'fd_bootstrap_ci -- category 12-distribution-risk, method card #196.',
    ...
    'fd_cullen_frey': {
        'fn': 'fd_cullen_frey',
        'description': 'fd_cullen_frey -- category 12-distribution-risk, method card #196.',
    ...
    'fd_fit': {
        'fn': 'fd_fit',
        'description': 'fd_fit -- category 12-distribution-risk, method card #196.',
    ...
    'fd_goodness_of_fit': {
        ...
        'description': 'fd_goodness_of_fit -- category 12-distribution-risk, method card #196.',
        'args': {'object': 'Handle to a fitdist fit (from fd_fit). Continuous -> KS/CvM/AD· discrete -> chisq (KS/CvM/AD = NA).'},
        'input_example': {'object': '<raw_handle>'},
    },
    'risk_component_var': {
        'fn': 'risk_component_var',
        'description': 'risk_component_var -- category 12-distribution-risk, method card #65.',
    ...
    'risk_downside': {
        'fn': 'risk_downside',
        'description': 'risk_downside -- category 12-distribution-risk, method card #65.',
    ...
    'risk_expected_shortfall': {
        ...
        'description': 'risk_expected_shortfall -- category 12-distribution-risk, method card #65.',
    ...
    'risk_returns_from_long': {
        ...
        'description': 'risk_returns_from_long -- category 12-distribution-risk, method card #65.',
    ...
    'risk_var': {
        'fn': 'risk_var',
        'description': 'risk_var -- category 12-distribution-risk, method card #65.',
    ...
    'qr_conditional_quantiles': {
        ...
        'description': 'qr_conditional_quantiles -- category 12-distribution-risk, method card #64.',
    ...
    'qr_growth_at_risk': {
        ...
        'description': 'qr_growth_at_risk -- category 12-distribution-risk, method card #64.',
    ...
    'qr_regression': {
        'fn': 'qr_regression',
        'description': 'qr_regression -- category 12-distribution-risk, method card #64.',
    ...
    'qr_slope_equality': {
        ...
        'description': 'qr_slope_equality -- category 12-distribution-risk, method card #64.',
        'args': {'object': 'Handle to an rqs fit with >=2 tau (anova.rq: equality of slopes across quantiles).'},
        'input_example': {'object': '<raw_handle>'},
    },
    'qrf_fit': {
        'fn': 'qrf_fit',
        'description': 'qrf_fit -- category 12-distribution-risk, method card #195.',
        'args': {'x': 'Handle to a matrix/DataFrame of predictors (one column per covariate· without NA).', 'y': 'Numeric response vector of length the row count of x (e.g. growth)· NOT factor, without NA.', 'ntree': 'Number of trees in the forest (default 200· small/deterministic).', 'nodesize': 'Minimum terminal node size (default 5· random-forest regression).', 'mtry': 'Predictors tried per split (default floor(p/3))· must be <= the column count of x.', 'seed': 'Reproducibility seed for the stochastic fit (default 2025).'},
        'input_example': {'x': '<matrix_handle>', 'y': [0.5, 0.5], 'ntree': 200, 'nodesize': 5, 'seed': 2025},
    },
    'qrf_predict': {
        'fn': 'qrf_predict',
        'description': 'qrf_predict -- category 12-distribution-risk, method card #195.',
        'args': {'object': 'Handle to a quantile-regression forest fit (from qrf_fit).', 'newdata': 'Handle to a matrix/DataFrame of new predictors (same columns as the training· without NA).', 'quantiles': 'Vector of quantile levels in [0,1]· passed WHOLE (e.g. [0.05,0.5,0.95]).'},
        'input_example': {'object': '<raw_handle>', 'newdata': '<matrix_handle>', 'quantiles': [0.5, 0.5]},
    },
    'st_density': {
        'fn': 'st_density',
        'description': 'st_density -- category 12-distribution-risk, method card #95.',
    ...
    'st_fit_quantiles': {
        'fn': 'st_fit_quantiles',
        'description': 'st_fit_quantiles -- category 12-distribution-risk, method card #95.',
    ...
    'st_shortfall': {
        'fn': 'st_shortfall',
        'description': 'st_shortfall -- category 12-distribution-risk, method card #95.',
    ...
    'vine_loglik': {
        'fn': 'vine_loglik',
        'description': 'vine_loglik -- category 12-distribution-risk, method card #198.',
    ...
    'vine_select': {
        'fn': 'vine_select',
        'description': 'vine_select -- category 12-distribution-risk, method card #198.',
    ...
    'vine_sim': {
        'fn': 'vine_sim',
        'description': 'vine_sim -- category 12-distribution-risk, method card #198.',
    ...
    'dr_var_backtest': {
        'fn': 'dr_var_backtest',
        'description': 'dr_var_backtest -- category 12-distribution-risk, method card #491.',
        'args': {'returns': 'Realised returns.', 'var_forecast': 'Forecast value at risk.', 'es_forecast': 'Forecast expected shortfall.', 'confidence': 'Tail level of the forecasts.', 'tests': 'Tests to run; omitted = the standard set.', 'alpha': 'Significance level.'},
        'input_example': {'returns': '<series_handle>', 'var_forecast': '<series_handle>', 'confidence': 0.99, 'alpha': 0.05},
    },
    'dr_caviar': {
        'fn': 'dr_caviar',
        'description': 'dr_caviar -- category 12-distribution-risk, method card #492.',
        'args': {'returns': 'Return series.', 'confidence': 'Tail level.', 'specification': 'CAViaR specification.', 'n_starts': 'Optimiser restarts.', 'seed': 'Seed for the random number generator.'},
        'input_example': {'returns': '<series_handle>', 'confidence': 0.99, 'specification': 'asymmetric_slope', 'n_starts': 10},
    },
    'dr_filtered_historical_simulation': {
        'fn': 'dr_filtered_historical_simulation',
        'description': 'dr_filtered_historical_simulation -- category 12-distribution-risk, method card #492.',
        'args': {'returns': 'Return series.', 'confidence': 'Tail level.', 'volatility_model': 'Volatility filter.', 'horizon': 'Forecast horizon.', 'n_simulations': 'Simulation draws.', 'seed': 'Seed for the random number generator; required for reproducibility.'},
        'input_example': {'returns': '<series_handle>', 'confidence': 0.99, 'volatility_model': 'garch', 'horizon': 1, 'n_simulations': 10000, 'seed': 1},
    },
    'dr_evt_threshold': {
        'fn': 'dr_evt_threshold',
        'description': 'dr_evt_threshold -- category 12-distribution-risk, method card #493.',
        'args': {'x': 'Data, usually losses.', 'thresholds': 'Candidate thresholds; omitted = a quantile grid.', 'min_exceedances': 'Minimum exceedances required.'},
        'input_example': {'x': '<series_handle>', 'min_exceedances': 30},
    },
    'dr_evt_fit': {
        'fn': 'dr_evt_fit',
        'description': 'dr_evt_fit -- category 12-distribution-risk, method card #493.',
        'args': {'x': 'Data, usually losses.', 'method': 'Approach.', 'threshold': 'Threshold for peaks over threshold.', 'block_size': 'Block size for block maxima.', 'return_periods': 'Return periods to report.', 'interval': 'Interval method.', 'conf_level': 'Confidence level for intervals.'},
        'input_example': {'x': '<series_handle>', 'method': 'pot', 'return_periods': [10.0, 50.0, 100.0], 'interval': 'profile', 'conf_level': 0.95},
    },
    'dr_l_moments': {
        'fn': 'dr_l_moments',
        'description': 'dr_l_moments -- category 12-distribution-risk, method card #494.',
        'args': {'x': 'Data.', 'distribution': 'Distribution to fit.', 'n_moments': 'L-moments computed.'},
        'input_example': {'x': '<series_handle>', 'distribution': 'gev', 'n_moments': 4},
    },
    'dr_regional_frequency': {
        'fn': 'dr_regional_frequency',
        'description': 'dr_regional_frequency -- category 12-distribution-risk, method card #494.',
        'args': {'data': 'Site-by-observation panel.', 'site': 'Site identifier.', 'value': 'Value column.', 'distribution': 'Regional distribution.', 'return_periods': 'Return periods to report.'},
        'input_example': {'data': '<df_handle>', 'site': '...', 'value': '...', 'distribution': 'gev', 'return_periods': [10.0, 50.0, 100.0]},
    },
    'dr_distributional_regression': {
        'fn': 'dr_distributional_regression',
        'description': 'dr_distributional_regression -- category 12-distribution-risk, method card #495.',
        'args': {'y': 'Outcome.', 'x': 'Covariate table.', 'family': 'Distribution family.', 'model_scale': 'Let covariates shift the scale.', 'model_shape': 'Let covariates shift the shape.', 'conf_level': 'Confidence level for intervals.'},
        'input_example': {'y': '<series_handle>', 'x': '<df_handle>', 'family': 'normal', 'model_scale': True, 'model_shape': False, 'conf_level': 0.95},
    },
    'dr_copula_fit': {
        'fn': 'dr_copula_fit',
        'description': 'dr_copula_fit -- category 12-distribution-risk, method card #496.',
        'args': {'u': 'Pseudo-observations on the unit cube.', 'families': 'Copula families to compare.', 'gof': 'Run the parametric-bootstrap goodness-of-fit test.', 'nboot': 'Number of bootstrap replications.', 'seed': 'Seed for the random number generator; required for reproducibility.'},
        'input_example': {'u': '<df_handle>', 'gof': True, 'nboot': 999, 'seed': 1},
    },
    'dr_tail_dependence': {
        'fn': 'dr_tail_dependence',
        'description': 'dr_tail_dependence -- category 12-distribution-risk, method card #496.',
        'args': {'u': 'Pseudo-observations on the unit cube.', 'method': 'Estimator.', 'conf_level': 'Confidence level for intervals.'},
        'input_example': {'u': '<df_handle>', 'method': 'nonparametric', 'conf_level': 0.95},
    },
    'dr_stress_scenarios': {
        'fn': 'dr_stress_scenarios',
        'description': 'dr_stress_scenarios -- category 12-distribution-risk, method card #497.',
        'args': {'factors': 'Risk-factor history.', 'method': 'Scenario construction.', 'episode': 'Historical episode to replay.', 'severity': 'Severity in standard deviations.', 'n_scenarios': 'Scenarios to generate.'},
        'input_example': {'factors': '<df_handle>', 'method': 'historical', 'severity': 3.0, 'n_scenarios': 10},
    },
    'dr_reverse_stress': {
        'fn': 'dr_reverse_stress',
        'description': 'dr_reverse_stress -- category 12-distribution-risk, method card #497.',
        'args': {'factors': 'Risk-factor history.', 'exposures': 'Portfolio sensitivities to the factors.', 'target_loss': 'Loss level to explain.'},
        'input_example': {'factors': '<df_handle>', 'exposures': '<series_handle>', 'target_loss': 1.0},
    },
    'dr_implied_density_input': {
        'fn': 'dr_implied_density_input',
        'description': 'dr_implied_density_input -- category 12-distribution-risk, method card #498.',
        'args': {'strikes': 'Strikes.', 'prices': 'Option prices.', 'forward': 'Forward price.', 'time_to_expiry': 'Time to expiry in years.', 'rate': 'Risk-free rate.', 'quantiles': 'Quantiles to extract.', 'risk_premium': 'Adjustment applied to convert to physical.'},
        'input_example': {'strikes': '<series_handle>', 'prices': '<series_handle>', 'forward': 1.0, 'time_to_expiry': 1.0, 'rate': 1.0, 'quantiles': [0.05, 0.25, 0.5, 0.75, 0.95], 'risk_premium': 0.0},
    },
}
