...
NODE_DOCS: dict[str, dict[str, Any]] = {
    ...
    'bss_fevd': {
        'fn': 'bss_fevd',
        'description': 'bss_fevd -- category 04-structural-shocks, METHOD-SELECTION card #149.',
    ...
    'bsv_hd': {
        'fn': 'bsv_hd',
        'description': 'bsv_hd -- category 04-structural-shocks, METHOD-SELECTION card #148.',
        'args': {'model': "Handle to a 'PosteriorBSVAR*' model.", 'probs': 'Quantile bands ∈ (0,1) (default 0.16/0.84).'},
        'input_example': {'model': '<raw_handle>'},
    },
    'bsv_irf': {
        'fn': 'bsv_irf',
        'description': 'bsv_irf -- category 04-structural-shocks, METHOD-SELECTION card #148.',
    ...
    'bsv_shocks': {
        'fn': 'bsv_shocks',
        'description': 'bsv_shocks -- category 04-structural-shocks, METHOD-SELECTION card #148.',
        'args': {'model': "Handle to a 'PosteriorBSVAR*' model.", 'probs': 'Quantile bands ∈ (0,1) (default 0.16/0.84).'},
        'input_example': {'model': '<raw_handle>'},
    },
    'bsv_verify': {
        'fn': 'bsv_verify',
        'description': 'bsv_verify -- category 04-structural-shocks, METHOD-SELECTION card #148.',
    ...
    'gmv_fit': {
        'fn': 'gmv_fit',
        'description': 'gmv_fit -- category 04-structural-shocks, METHOD-SELECTION card #151.',
        'args': {'data': 'Handle to a multivariate series (matrix/ts, T x d, d>=2, no NA/Inf).', 'p': 'VAR lag order (positive integer, default 1).', 'M': 'Number of regimes· positive integer (GMVAR/StMVAR) or a length-2 vector [M1,M2] for G-StMVAR (default 2).', 'model': "Mixture type: Gaussian (GMVAR), Student's-t (StMVAR), or mixed G-StMVAR (default GMVAR).", 'conditional': 'Conditional (True) or exact (False) log-likelihood (default True).', 'parametrization': 'Intercept or mean parametrization (default intercept).', 'structural_W': 'Optional d x d constraint matrix W for structural identification through conditional heteroskedasticity (NA=free, +/-=sign, 0=zero)· requires M>=2. None => reduced-form (recursive Cholesky).', 'ncalls': 'Number of GA estimation rounds (default 2 — TINY· production raises it to (M+1)^5).', 'ncores': 'CPU cores (default 1· use_parallel only when >1).', 'maxit': 'Maximum variable-metric optim iterations per round (default 100).', 'seed': 'MANDATORY seed (integer) — stochastic GA· set.seed + reproducible seeds vector.', 'allow_nonconvergence': 'True to bypass the convergence gate (degenerate estimates· the IRF/FEVD will be UNRELIABLE· default False).'},
    ...
    'gmv_girf': {
        'fn': 'gmv_girf',
        'description': 'gmv_girf -- category 04-structural-shocks, METHOD-SELECTION card #151.',
        'args': {'model': "Handle to a fitted 'gsmvar' model (from gmv_fit).", 'which_shocks': 'Indices of the structural shocks in [1, d] (default 1).', 'N': 'GIRF horizon (positive integer, default 10).', 'n_iterations': 'MC iterations per initial value (default 50 — TINY).', 'n_initial_values': 'Number of initial values from the stationary distribution (default 50 — TINY).', 'shock_size': 'Size/sign of the shock· non-zero (default 1).', 'ci': 'Confidence levels of the bands in (0,1) (default [0.95, 0.8]).', 'include_mixweights': 'GIRF for the mixing weights as well (default False).', 'ncores': 'CPU cores (default 1).', 'seed': 'MANDATORY seed — simulation-based· set.seed for reproducibility.'},
    ...
    'lp_linear_iv': {
        'fn': 'lp_linear_iv',
        'description': 'lp_linear_iv -- category 04-structural-shocks, METHOD-SELECTION card #22.',
    ...
    'lp_nonlinear': {
        'fn': 'lp_nonlinear',
        'description': 'lp_nonlinear -- category 04-structural-shocks, METHOD-SELECTION card #22.',
    ...
    'lp_nonlinear_iv': {
        'fn': 'lp_nonlinear_iv',
        'description': 'lp_nonlinear_iv -- category 04-structural-shocks, METHOD-SELECTION card #22.',
    ...
    'lp_panel': {
        'fn': 'lp_panel',
        'description': 'lp_panel -- category 04-structural-shocks, METHOD-SELECTION card #22.',
        'args': {'data_set': 'Handle to a panel DataFrame (1st column cross-section, 2nd time, >=3 columns).', 'endog_data': 'NAME of the dependent-variable column (not the data).', 'shock': 'NAME of the shock column (not the data).', 'hor': 'IRF horizon (default 10).', 'confint': 'CI multiplier (default 1.96).', 'panel_model': 'plm estimator (default within = fixed effects).', 'panel_effect': 'plm effect dimension (default individual).', 'iv_reg': 'IV/GMM (requires instrum) (default False).', 'cumul_mult': 'Cumulative multipliers (default True).', 'diff_shock': 'First-difference the shock (default True).'},
    ...
    'lp_panel_nl': {
        'fn': 'lp_panel_nl',
        'description': 'lp_panel_nl -- category 04-structural-shocks, METHOD-SELECTION card #22.',
        'args': {'data_set': 'Handle to a panel DataFrame (cross-section, time, >=3 columns).', 'endog_data': 'NAME of the dependent-variable column.', 'shock': 'NAME of the shock column.', 'switching': 'NAME of the regime-variable column.', 'hor': 'IRF horizon (default 10).', 'confint': 'CI multiplier (default 1.96).', 'panel_model': 'plm estimator (default within).', 'panel_effect': 'plm effect dimension (default individual).', 'cumul_mult': 'Cumulative multipliers (default True).', 'diff_shock': 'First-difference the shock (default True).', 'use_logistic': 'Logistic transition (default True).', 'use_hp': 'HP-filter the switching variable (requires lambda) (default False).', 'lambda': 'HP lambda· required if use_hp=True.', 'gamma': 'Logistic steepness gamma (default 3).'},
    ...
    'sst_fit': {
        'fn': 'sst_fit',
        'description': 'sst_fit -- category 04-structural-shocks, METHOD-SELECTION card #150.',
        'args': {'data': 'Handle to a multivariate series (matrix/ts, T x d, d>=2, no NA/Inf).', 'p': 'VAR lag order (positive integer, default 1).', 'M': 'Number of regimes (positive integer, default 2).', 'weight_function': 'Transition-weight function (default relative_dens — Gaussian ONLY).', 'cond_dist': 'Conditional error distribution (default Gaussian).', 'parametrization': 'Intercept or mean parametrization (default intercept).', 'weightfun_pars': 'Weight-function parameters· [switch_var, lag] for logistic/exponential/threshold (mandatory when weight_function != relative_dens).', 'nrounds': 'Number of GA estimation rounds (default 2 — TINY· production raises it).', 'ngen': 'Genetic-algorithm generations per round (default 30 — TINY).', 'maxit': 'Maximum variable-metric optim iterations (default 300).', 'seed': 'MANDATORY seed (integer) — stochastic GA· set.seed + seeds vector.'},
        'input_example': {'data': '<multiseries_handle>', 'p': 1, 'M': 2, 'nrounds': 2, 'ngen': 30, 'maxit': 300, 'seed': 1},
    },
    'sst_gfevd': {
        'fn': 'sst_gfevd',
        'description': 'sst_gfevd -- category 04-structural-shocks, METHOD-SELECTION card #150.',
        'args': {'model': "Handle to a fitted 'stvar' model (from sst_fit).", 'N': 'GFEVD horizon (positive integer, default 16).', 'shock_size': 'Size/sign of the shocks (default 1).', 'initval_type': 'Type of initial values for the GIRFs (default random).', 'n_iterations': 'MC iterations per initial value (default 50 — TINY).', 'n_initial_values': 'Number of initial values (random/fixed) (default 50 — TINY).', 'init_regime': 'Regime of the initial values 1..M (default 1).', 'burn_in': 'Simulation burn-in (default 200).', 'seed': 'MANDATORY seed — simulation-based· set.seed for reproducibility.'},
        'input_example': {'model': '<raw_handle>', 'N': 16, 'shock_size': 1, 'n_iterations': 50, 'n_initial_values': 50, 'init_regime': 1, 'burn_in': 200, 'seed': 1},
    },
    'sst_girf': {
        'fn': 'sst_girf',
        'description': 'sst_girf -- category 04-structural-shocks, METHOD-SELECTION card #150.',
        'args': {'model': "Handle to a fitted 'stvar' model (from sst_fit).", 'which_shocks': 'Indices of the structural shocks in [1, d] (default 1).', 'N': 'GIRF horizon (positive integer, default 16).', 'shock_size': 'Size/sign of the shock (default 1).', 'n_iterations': 'MC iterations per initial value (default 50 — TINY).', 'n_initial_values': 'Number of initial values from init_regime (default 50 — TINY).', 'init_regime': 'Regime generating the initial values 1..M (default 1).', 'ci': 'Confidence levels of the bands (default [0.95, 0.8]).', 'burn_in': 'Simulation burn-in (default 200).', 'seed': 'MANDATORY seed — simulation-based· set.seed for reproducibility.'},
        'input_example': {'model': '<raw_handle>', 'N': 16, 'shock_size': 1, 'n_iterations': 50, 'n_initial_values': 50, 'init_regime': 1, 'burn_in': 200, 'seed': 1},
    },
    'sst_linear_irf': {
        'fn': 'sst_linear_irf',
        'description': 'sst_linear_irf -- category 04-structural-shocks, METHOD-SELECTION card #150.',
    ...
    'svar_cf': {
        'fn': 'svar_cf',
        'description': 'svar_cf -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'svar_fevd': {
        'fn': 'svar_fevd',
        'description': 'svar_fevd -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'svar_hd': {
        'fn': 'svar_hd',
        'description': 'svar_hd -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'svar_id_chol': {
        'fn': 'svar_id_chol',
        'description': 'svar_id_chol -- category 04-structural-shocks, METHOD-SELECTION card #20.',
        'args': {'model': "Handle to a reduced-form 'varest' (recursive/Cholesky ID)."},
        'input_example': {'model': '<raw_handle>'},
    },
    'svar_id_cv': {
        'fn': 'svar_id_cv',
        'description': 'svar_id_cv -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'svar_id_dc': {
        'fn': 'svar_id_dc',
        'description': 'svar_id_dc -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'svar_id_ngml': {
        'fn': 'svar_id_ngml',
        'description': 'svar_id_ngml -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'svar_irf': {
        'fn': 'svar_irf',
        'description': 'svar_irf -- category 04-structural-shocks, METHOD-SELECTION card #20.',
        'args': {'x': "Handle to an identified 'svars' (from svar_id_*).", 'n_ahead': 'IRF horizon (positive integer, default 10).'},
        'input_example': {'x': '<raw_handle>', 'n_ahead': 10},
    },
    'svar_mb_boot': {
        'fn': 'svar_mb_boot',
        'description': 'svar_mb_boot -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'svar_wild_boot': {
        'fn': 'svar_wild_boot',
        'description': 'svar_wild_boot -- category 04-structural-shocks, METHOD-SELECTION card #20.',
    ...
    'vr_bq': {
        'fn': 'vr_bq',
        'description': 'vr_bq -- category 04-structural-shocks, METHOD-SELECTION card #19.',
        'args': {'model': "Handle to a reduced-form 'varest' (BQ long-run restrictions)."},
        'input_example': {'model': '<raw_handle>'},
    },
    'vr_fevd': {
        'fn': 'vr_fevd',
        'description': 'vr_fevd -- category 04-structural-shocks, METHOD-SELECTION card #19.',
    ...
    'vr_irf': {
        'fn': 'vr_irf',
        'description': 'vr_irf -- category 04-structural-shocks, METHOD-SELECTION card #19.',
    ...
    'vr_svar': {
        'fn': 'vr_svar',
        'description': 'vr_svar -- category 04-structural-shocks, METHOD-SELECTION card #19.',
        'args': {'model': "Handle to a reduced-form 'varest' (from vr_var, button #11).", 'estmethod': 'Estimation method (default scoring).', 'Amat': 'Handle to a K x K A-model matrix (NA = free parameter).', 'Bmat': 'Handle to a K x K B-model matrix (NA = free parameter).', 'lrtest': 'LR test overidentifying restrictions (default True).'},
        'input_example': {'model': '<raw_handle>', 'lrtest': True},
    },
    'vsr_rwz_reject': {
        'fn': 'vsr_rwz_reject',
        'description': 'vsr_rwz_reject -- category 04-structural-shocks, METHOD-SELECTION card #21.',
    ...
    'vsr_uhlig_penalty': {
        ...
        'description': 'vsr_uhlig_penalty -- category 04-structural-shocks, METHOD-SELECTION card #21.',
    ...
    'vsr_uhlig_reject': {
        'fn': 'vsr_uhlig_reject',
        'description': 'vsr_uhlig_reject -- category 04-structural-shocks, METHOD-SELECTION card #21.',
