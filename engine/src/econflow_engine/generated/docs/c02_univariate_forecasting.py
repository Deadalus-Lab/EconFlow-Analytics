...
NODE_DOCS: dict[str, dict[str, Any]] = {
    'caf_arml': {
        'fn': 'caf_arml',
        'description': 'caf_arml -- category 02-univariate-forecasting, METHOD-SELECTION card #134.',
        'args': {'y': 'Handle to a univariate ts (the series to forecast).', 'seed': 'REQUIRED seed (set.seed before caret resampling; cache key; uncacheable without it).', 'max_lag': 'Maximum lag of the autoregressive features (positive integer; default 5; length(y)>max_lag+1).', 'caret_method': 'CLOSED whitelist of lightweight learners (default lm); maps internally to caret method+tune_grid; NEVER a free string.', 'pre_process': 'Optional preprocessing steps (center/scale/range/BoxCox etc.; gated to an allowed set; None=none).', 'cv': 'Model selection with time-slice CV (default True; cv=False is rejected — requires a forbidden tune_grid).', 'cv_horizon': 'Number of consecutive values in the test set of each resample (positive integer; default 4).', 'seasonal': 'Fourier terms for seasonality (default True).', 'exog': 'In-sample exogenous regressors (matrix, nrow==length(y); NOT a data frame).', 'new_exog': 'Future exogenous regressors for the forecast (nrow==h, same columns as exog_handle; required if exog_handle is given).', 'h': 'Forecast horizon (positive integer; None -> frequency(y)).'},
        'input_example': {'y': '<series_handle>', 'seed': 1},
    },
    'caf_conformal': {
        'fn': 'caf_conformal',
        'description': 'caf_conformal -- category 02-univariate-forecasting, METHOD-SELECTION card #134.',
        'args': {'model': 'Handle to an ARml model (caf_arml$model) — conformal PI from its in-sample residuals.', 'h': 'Forecast horizon (positive integer; None -> frequency(model$y)).', 'confidence': 'Confidence level of the conformal interval ∈ (0,1) (default 0.95).', 'new_exog': 'Future regressors if the model was trained with exog_handle (NOT a data frame).', 'y_min': 'Lower clip of the intervals (default -Inf).', 'y_max': 'Upper clip of the intervals (> y_min; default Inf).'},
        'input_example': {'model': '<raw_handle>'},
    },
    'caf_varimp': {
        'fn': 'caf_varimp',
        'description': 'caf_varimp -- category 02-univariate-forecasting, METHOD-SELECTION card #134.',
        'args': {'model': 'Handle to an ARml or forecast object (caf_arml) — variable importance of the lags/regressors.'},
        'input_example': {'model': '<raw_handle>'},
    },
    'dlm_ardl': {
        'fn': 'dlm_ardl',
        'description': 'dlm_ardl -- category 02-univariate-forecasting, METHOD-SELECTION card #137.',
    ...
    'dlm_finite': {
        'fn': 'dlm_finite',
        'description': 'dlm_finite -- category 02-univariate-forecasting, METHOD-SELECTION card #137.',
    ...
    'dlm_koyck': {
        'fn': 'dlm_koyck',
        'description': 'dlm_koyck -- category 02-univariate-forecasting, METHOD-SELECTION card #137.',
    ...
    'dlm_poly': {
        'fn': 'dlm_poly',
        'description': 'dlm_poly -- category 02-univariate-forecasting, METHOD-SELECTION card #137.',
    ...
    'fdf_diffseries': {
        'fn': 'fdf_diffseries',
        'description': 'fdf_diffseries -- category 02-univariate-forecasting, METHOD-SELECTION card #136.',
    ...
    'fdf_fit': {
        'fn': 'fdf_fit',
        'description': 'fdf_fit -- category 02-univariate-forecasting, METHOD-SELECTION card #136.',
    ...
    'fdf_gph': {
        'fn': 'fdf_gph',
        'description': 'fdf_gph -- category 02-univariate-forecasting, METHOD-SELECTION card #136.',
        'args': {'x': 'Handle to a univariate ts/numeric.', 'bandw_exp': 'Bandwidth exponent bw=trunc(n^bandw.exp); strictly in (0,1); default 0.5.'},
        'input_example': {'x': '<series_handle>'},
    },
    'fdf_sperio': {
        'fn': 'fdf_sperio',
        'description': 'fdf_sperio -- category 02-univariate-forecasting, METHOD-SELECTION card #136.',
    ...
    'meb_ensemble': {
        'fn': 'meb_ensemble',
        'description': 'meb_ensemble -- category 02-univariate-forecasting, METHOD-SELECTION card #138.',
        'args': {'x': 'Handle to a univariate series (numeric/ts; fully finite, length>=2).', 'seed': 'Seed (REQUIRED; stochastic — set.seed before the draw; reproducibility/caching).', 'reps': 'Number of bootstrap replicates (default 999; gate reps>=10).', 'trim': 'Trimming proportion of the tails (default 0.10; gate 0<=trim<0.5).', 'reachbnd': 'Reached-bounds draws at the 0/1 edges (default True).', 'expand_sd': "Widening of the ensemble's SD (default True).", 'force_clt': 'Forcing of CLT on the ensemble mean (default True).', 'scl_adjustment': 'Scale adjustment so that var(ensemble)=var(x) (default False).', 'sym': 'Symmetric ME density adjustment (default False).'},
        'input_example': {'x': '<series_handle>', 'seed': 1},
    },
    'nnf_elm': {
        'fn': 'nnf_elm',
        'description': 'nnf_elm -- category 02-univariate-forecasting, METHOD-SELECTION card #133.',
        'args': {'y': 'Handle to a univariate ts/msts.', 'seed': 'REQUIRED seed (set.seed before the random hidden weights; cache key).', 'h': 'Forecast horizon (positive integer; None -> frequency(y)).', 'hd': 'Number of hidden nodes (small default 3).', 'type': 'Estimation of output-layer weights (default lasso).', 'reps': 'Ensemble size (default 2).', 'comb': 'Ensemble combination operator (default median).', 'sel_lag': 'Automatic lag selection (default True).', 'direct': 'Direct input-output connections for linear effects (default False).', 'det_type': 'Type of deterministic seasonal dummies (default auto).', 'exog': 'Exogenous regressors (matrix, length >= n+h; NOT a data frame).'},
        'input_example': {'y': '<series_handle>', 'seed': 1},
    },
    'nnf_mlp': {
        'fn': 'nnf_mlp',
        'description': 'nnf_mlp -- category 02-univariate-forecasting, METHOD-SELECTION card #133.',
        'args': {'y': 'Handle to a univariate ts/msts (the series to forecast).', 'seed': 'REQUIRED seed (set.seed before the random weights; cache key; uncacheable without it).', 'h': 'Forecast horizon (positive integer; None -> frequency(y)).', 'hd': 'Number of hidden nodes (small default 3; keeps the node cheap).', 'reps': 'Ensemble size (number of networks; default 2; docs 20).', 'comb': 'Ensemble combination operator (default median).', 'sel_lag': 'Automatic lag selection (default True).', 'det_type': 'Type of deterministic seasonal dummies (default auto).', 'exog': 'Exogenous regressors (matrix, length >= n+h; NOT a data frame).'},
        'input_example': {'y': '<series_handle>', 'seed': 1},
    },
    'run_adam': {
        'fn': 'run_adam',
        'description': 'run_adam -- category 02-univariate-forecasting, METHOD-SELECTION card #10.',
        'args': {'data': 'Handle to a ts (1st column = response; data-agnostic passthrough).', 'model': "ETS spec E/T/S (default 'ZXZ'; Z=auto, X=pure additive selection).", 'h': 'Fit-time horizon (for holdout; default 0).', 'holdout': 'Keep the last h points as holdout (requires h>0; default False).', 'loss': 'Loss function (default likelihood).', 'ic': 'Information criterion (default AICc).', 'forecast_h': 'Horizon of the forecast step (None -> fit h if >0, otherwise 10).'},
        'input_example': {'data': '<series_handle>'},
    },
    'run_auto_adam': {
        'fn': 'run_auto_adam',
        'description': 'run_auto_adam -- category 02-univariate-forecasting, METHOD-SELECTION card #10.',
        'args': {'data': 'Handle to a ts (auto.adam tries multiple distributions).', 'model': "ETS spec E/T/S (default 'ZXZ').", 'h': 'Fit-time horizon (default 0).', 'holdout': 'Holdout of the last h (requires h>0; default False).', 'ic': 'Information criterion (default AICc).', 'forecast_h': 'Horizon of the forecast step.'},
        'input_example': {'data': '<series_handle>'},
    },
    'run_auto_arima': {
        'fn': 'run_auto_arima',
        'description': 'run_auto_arima -- category 02-univariate-forecasting, METHOD-SELECTION card #7.',
    ...
    'run_auto_ces': {
        'fn': 'run_auto_ces',
        'description': 'run_auto_ces -- category 02-univariate-forecasting, METHOD-SELECTION card #10.',
        'args': {'y': 'Handle to a ts (auto.ces tries the seasonality types).', 'ic': 'Information criterion (default AICc).', 'loss': 'Loss function (default likelihood).', 'h': 'Fit-time horizon (default 0).', 'holdout': 'Holdout of the last h (requires h>0; default False).', 'forecast_h': 'Horizon of the forecast step.'},
        'input_example': {'y': '<series_handle>'},
    },
    'run_ces': {
        'fn': 'run_ces',
        'description': 'run_ces -- category 02-univariate-forecasting, METHOD-SELECTION card #10.',
        'args': {'y': 'Handle to a ts (univariate response).', 'seasonality': 'Type of complex seasonality (default none).', 'loss': 'Loss function (default likelihood).', 'h': 'Fit-time horizon (default 0).', 'holdout': 'Holdout of the last h (requires h>0; default False).', 'forecast_h': 'Horizon of the forecast step.'},
        'input_example': {'y': '<series_handle>'},
    },
    'run_ets': {
        'fn': 'run_ets',
        'description': 'run_ets -- category 02-univariate-forecasting, METHOD-SELECTION card #7.',
    ...
    'run_fable_arima': {
        'fn': 'run_fable_arima',
        'description': 'run_fable_arima -- category 02-univariate-forecasting, METHOD-SELECTION card #8.',
        'args': {'data': 'Handle to a series (index + value variable).', 'formula': "Model spec, e.g. 'value ~ 1' (automatic order selection). NOTE: the special fable functions pdq/PDQ are NOT allowed by the security allowlist.", 'ic': 'Information criterion (default aicc).', 'stepwise': 'Stepwise search (default True).', 'h': 'Forecast horizon.'},
        'input_example': {'data': '<df_handle>', 'formula': 'y ~ x'},
    },
    'run_fable_ets': {
        'fn': 'run_fable_ets',
        'description': 'run_fable_ets -- category 02-univariate-forecasting, METHOD-SELECTION card #8.',
        'args': {'data': 'Handle to a series (WITHOUT missing values — ETS does not support them).', 'formula': 'ETS model spec; NOTE: the fable functions error/trend/season are NOT allowed by the security allowlist.', 'opt_crit': 'Optimization criterion (default lik).', 'ic': 'Information criterion (default aicc).', 'h': 'Forecast horizon.'},
        'input_example': {'data': '<df_handle>', 'formula': 'y ~ x'},
    },
    'run_prophet': {
        'fn': 'run_prophet',
        'description': 'run_prophet -- category 02-univariate-forecasting, METHOD-SELECTION card #9.',
    ...
    'run_prophet_cv': {
        'fn': 'run_prophet_cv',
        'description': 'run_prophet_cv -- category 02-univariate-forecasting, METHOD-SELECTION card #9.',
    ...
    'run_tbats': {
        'fn': 'run_tbats',
        'description': 'run_tbats -- category 02-univariate-forecasting, METHOD-SELECTION card #7.',
    ...
    'run_theta': {
        'fn': 'run_theta',
        'description': 'run_theta -- category 02-univariate-forecasting, METHOD-SELECTION card #7.',
        'args': {'y': 'Handle to a univariate ts.', 'h': 'Forecast horizon.'},
        'input_example': {'y': '<series_handle>'},
    },
    'tsi_croston': {
        'fn': 'tsi_croston',
        'description': 'tsi_croston -- category 02-univariate-forecasting, METHOD-SELECTION card #135.',
    ...
    'tsi_decomp': {
        'fn': 'tsi_decomp',
        'description': 'tsi_decomp -- category 02-univariate-forecasting, METHOD-SELECTION card #135.',
    ...
    'tsi_imapa': {
        'fn': 'tsi_imapa',
        'description': 'tsi_imapa -- category 02-univariate-forecasting, METHOD-SELECTION card #135.',
    ...
    'tsi_tsb': {
        'fn': 'tsi_tsb',
        'description': 'tsi_tsb -- category 02-univariate-forecasting, METHOD-SELECTION card #135.',
