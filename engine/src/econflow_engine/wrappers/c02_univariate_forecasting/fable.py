# SPDX-License-Identifier: AGPL-3.0-only
"""Method wrapper ``fable`` -- METHOD-SELECTION card #8.

#8 fable (tidy ARIMA/ETS)

Category 02-univariate-forecasting; module ``fable``.

Reference implementation: not yet selected; see engine/METHOD-SOURCES.json.

See ``./README.md`` for when this method applies, what to reach for instead, and the interpretation
traps recorded against it.
"""

from __future__ import annotations

from typing import TYPE_CHECKING, Any, Literal

from econflow_engine.generated.args.c02_univariate_forecasting import NODE_META, wire_model

if TYPE_CHECKING:
    import pandas as pd

# Re-exported so a body can re-validate its own inputs with ``wire_model(fn)`` and
# read kinds and defaults from ``NODE_META[fn]`` without another import.
__all__ = [
    "run_fable_arima",
    "run_fable_ets",
    "NODE_META",
    "wire_model",
]


def run_fable_arima(
    *,
    data: pd.DataFrame,
    formula: str,
    ic: Literal["aicc", "aic", "bic"] | None = None,
    stepwise: bool | None = None,
    h: int | None = None,
) -> dict[str, Any]:
    """Node ``run_fable_arima`` -- METHOD-SELECTION card #8.

    fable (tidy ARIMA/ETS).

    Category 02-univariate-forecasting; memory class ``light``.

    Args:
        data: [df_handle, required] Handle to a series (index + value variable).
        formula: [formula, required] Model spec, e.g. 'value ~ 1' (automatic order selection). NOTE:
            the special fable functions pdq/PDQ are NOT allowed by the security allowlist.
        ic: [enum, optional] Information criterion (default aicc).
        stepwise: [boolean, optional] Stepwise search (default True).
        h: [integer, optional] Forecast horizon.

    Returns:
        A JSON-safe mapping, ready for ``econflow_engine.serialize.to_mcp``.
    """
    raise NotImplementedError(
        "run_fable_arima: not implemented. The method card is in ./README.md."
    )


def run_fable_ets(
    *,
    data: pd.DataFrame,
    formula: str,
    opt_crit: Literal["lik", "amse", "mse", "sigma", "mae"] | None = None,
    ic: Literal["aicc", "aic", "bic"] | None = None,
    h: int | None = None,
) -> dict[str, Any]:
    """Node ``run_fable_ets`` -- METHOD-SELECTION card #8.

    fable (tidy ARIMA/ETS).

    Category 02-univariate-forecasting; memory class ``light``.

    Args:
        data: [df_handle, required] Handle to a series (WITHOUT missing values — ETS does not
            support them).
        formula: [formula, required] ETS model spec; NOTE: the fable functions error/trend/season
            are NOT allowed by the security allowlist.
        opt_crit: [enum, optional] Optimization criterion (default lik).
        ic: [enum, optional] Information criterion (default aicc).
        h: [integer, optional] Forecast horizon.

    Returns:
        A JSON-safe mapping, ready for ``econflow_engine.serialize.to_mcp``.
    """
    raise NotImplementedError(
        "run_fable_ets: not implemented. The method card is in ./README.md."
    )
