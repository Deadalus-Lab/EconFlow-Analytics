...
"""Method wrapper ``computable_applied_general`` -- METHOD-SELECTION card #202.
...
Category 13-dsge-general-equilibrium; module ``computable_applied_general``.
...
def cge_solve(
    *,
    ...
    """Node ``cge_solve`` -- METHOD-SELECTION card #202.

    Computable/Applied General Equilibrium — a structural dynamic model (sdm) with a
    Leontief/Cobb-Douglas/CES demand structure; equilibrium prices, activity levels, growth rate.

    Category 13-dsge-general-equilibrium; memory class ``heavy``.
    ...
    raise NotImplementedError(
        "cge_solve: not implemented. The method card is in ./README.md."
    )
