...
"""Method wrapper ``diebold_mariano_tscv`` -- METHOD-SELECTION card #74.

#74 Diebold-Mariano test + tsCV rolling backtest

Category 15-model-evaluation; module ``diebold_mariano_tscv``.
...
def eval_dm_test(
    *,
    ...
    """Node ``eval_dm_test`` -- METHOD-SELECTION card #74.

    Diebold-Mariano test + tsCV rolling backtest.
    ...
    raise NotImplementedError(
        "eval_dm_test: not implemented. The method card is in ./README.md."
    )
...
def eval_tsCV(
    *,
    ...
    """Node ``eval_tsCV`` -- METHOD-SELECTION card #74.
    ...
    Args:
        ...
        forecastfunction: [forecastfn_enum, required] The model/pipeline that is backtested:
            ...
            The classic benchmark (FPP3). · snaive = Seasonal naive: f(t+k)=y(T+k-m). Benchmark for
            seasonal series; at frequency=1 it degenerates IDENTICALLY to naive (lag=1). · drift =
            Random walk with drift: naive + mean historical change (FPP3 benchmark). · mean = Mean
            forecast: f(t+k)=mean(y). Benchmark; bootstrap=True (5000 stochastic paths) is PINNED to
            False for determinism. · ses = Simple exponential smoothing = ETS(A,N,N) (without
            trend/seasonality). · holt = Holt linear trend = ETS(A,A,N) (linear trend, without
            seasonality). · ets = Automatic ETS state-space (error/trend/season selection with
            AICc); same method as the run_ets node (cat. 02). · auto_arima = Automatic (S)ARIMA with
            stepwise Hyndman-Khandakar; same method as the run_auto_arima node (cat. 02). · theta =
            Theta method (Assimakopoulos & Nikolopoulos 2000, M3 winner); same family as the
            run_theta node (cat. 02). Caution: tsCV re-fits at EVERY origin -> ets/auto_arima are
            significantly slower than the benchmarks.
    ...
    """
    raise NotImplementedError(
        "eval_tsCV: not implemented. The method card is in ./README.md."
    )
