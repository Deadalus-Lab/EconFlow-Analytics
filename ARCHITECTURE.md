...
| Infrastructure modules | **28** | `find src -name '*.py' -not -path '*__pycache__*' -not -path 'src/econflow_engine/wrappers/*' -not -path 'src/econflow_engine/generated/*' \| wc -l` |
| Test modules | **21** | `find tests -name 'test_*.py' -not -path '*__pycache__*' \| wc -l` |
...
| Python files with an SPDX header | **812** | `find src scripts tests -name '*.py' -not -path '*__pycache__*' -print0 \| xargs -0 grep -l 'SPDX-License-Identifier' \| wc -l` |
