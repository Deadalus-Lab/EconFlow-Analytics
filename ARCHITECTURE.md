...
| Test modules | **32** | `find tests -name 'test_*.py' -not -path '*__pycache__*' \| wc -l` |
...
| Python files with an SPDX header | **829** | `find src scripts tests -name '*.py' -not -path '*__pycache__*' -print0 \| xargs -0 grep -l 'SPDX-License-Identifier' \| wc -l` |
